...
def draw_histogram(stone_brightnesses):
    ...
    # Clear the subplot to remove any previous drawings
    black_thresh_subfigure.clear()

    # Check if the board is ready; if not, refresh the canvas and return
    if not board_ready:
        black_thresh_hist.draw()
        return

    # Draw histogram
    threshold_hist = black_thresh_subfigure.hist(
        stone_brightnesses, bins=20, range=(0, 255), color="pink"
    )

    # Determine the maximum value for scaling
    max_val = max(threshold_hist[0]) if len(threshold_hist[0]) > 0 else 0

    # Update or draw the threshold line
    if threshold_line:
        for line in threshold_line:
            line.set_data([black_stone_threshold, black_stone_threshold], [0, max_val])
    else:
        threshold_line = black_thresh_subfigure.plot(
            [black_stone_threshold, black_stone_threshold],
            [0, max_val],
            color="red",
        )

    # Add text annotations
    black_thresh_subfigure.text(
        black_stone_threshold,
        max_val * 0.95,
        str(black_stone_threshold),
        fontsize=8,
    )
    black_thresh_subfigure.text(
        black_stone_threshold - 70,
        max_val * 0.8,
        f"{num_black_stones} black",
        fontsize=8,
    )
    black_thresh_subfigure.text(
        black_stone_threshold + 10,
        max_val * 0.8,
        f"{num_white_stones} white",
        fontsize=8,
    )
